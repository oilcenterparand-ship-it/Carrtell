...
test.describe('Persona: Admin', () => {
  ...
  test('authenticated admin can open critical management sections', async ({ page }, testInfo) => {
    ...
    expect(username, 'CARRTELL_ADMIN_USERNAME is required; agent-category.ps1 prompts for it.').not.toBe('');
    expect(password, 'CARRTELL_ADMIN_PASSWORD is required; agent-category.ps1 prompts for it securely.').not.toBe('');
    ...
    for (const route of adminRoutes) {
      ...
      if (route === '/admin/home-content') await expect(page.getByTestId('admin-mega-menu-promotion-form')).toBeVisible();
    }
  });
});
