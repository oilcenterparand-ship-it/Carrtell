...
create policy mega_menu_tiles_public_read on public.mega_menu_tiles for select to anon, authenticated using (is_active or public.is_admin());
drop policy if exists mega_menu_tiles_admin_manage on public.mega_menu_tiles;
create policy mega_menu_tiles_admin_manage on public.mega_menu_tiles for all to authenticated using (public.is_admin()) with check (public.is_admin());
