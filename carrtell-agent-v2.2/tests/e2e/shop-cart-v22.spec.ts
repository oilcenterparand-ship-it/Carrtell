...
test.describe('Carrtell v2.2 shop/cart customer journey', () => {
  ...
  test('cart product detail links use the real product route', async ({ page }) => {
    await page.goto('/cart');
    const link = page.getByRole('link', { name: 'QA Carrtell Product' }).first();
    await expect(link).toHaveAttribute('href', '/shop/product/qa-product-1');
  });

  test('guest checkout does not demand account before payment step', async ({ page }) => {
    await page.goto('/cart');

    const continueButton = page.getByRole('button', { name: /ادامه ثبت سفارش/ }).first();
    await expect(continueButton).toBeVisible();
    await continueButton.click();

    const inputs = page.locator('input');
    const nameInput = page.getByPlaceholder(/امین|نام/).first();
    const phoneInput = page.getByPlaceholder(/09/).first();
    ...
    // choose pickup to avoid address/network dependencies
    await page.getByRole('button', { name: /تحویل حضوری/ }).click();

    const goPayment = page.getByRole('button', { name: /ادامه پرداخت/ });
    await expect(goPayment).toBeEnabled();
    await goPayment.click();

    // Product requirement: guest should reach payment flow without being forced
    // to create/login to an account before payment.
    expect(page.url(), 'guest was forced to OTP before payment').not.toContain('/login-otp');
  });
  ...
  test('mobile shop/cart have no major horizontal overflow', async ({ page }, testInfo) => {
    ...
    for (const route of ['/shop', '/cart']) {
      ...
      expect(d.sw - d.cw, `${route} overflow = ${d.sw-d.cw}px`).toBeLessThanOrEqual(6);
    }
  });
});
