...
export type DriverJobReportInput = { finalKm: number; usedProducts?: string; notes?: string };
...
export async function completeDriverJob(id: string, report: DriverJobReportInput) {
  ...
  await createServiceHistoryFromJob(normalized).catch(() => undefined);
  return normalized;
}

async function createServiceHistoryFromJob(job: DriverJob) {
  if (!job.id || !job.final_km) return;
  await supabase.from('service_history').insert({ service_request_id: job.id, customer_name: job.customer_name ?? null, customer_phone: job.customer_phone ?? null, car_name: job.car_name ?? null, service_km: job.final_km, next_service_km: job.next_service_km ?? null, used_products: job.used_products ?? [], notes: job.driver_notes ?? null, created_at: new Date().toISOString() });
}
