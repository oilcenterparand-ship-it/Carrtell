            ...
            return <section key={task.id} className="overflow-hidden rounded-3xl border border-white/10 bg-slate-900/90 shadow-xl">
              <div className="p-5">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div><div className="text-lg font-black">{task.customer_name || 'مشتری'} — {task.vehicle_title || task.car_name || 'خودرو نامشخص'}</div><div className="mt-1 text-sm text-slate-400">{task.customer_phone} • کیلومتر ثبت‌شده: {task.current_km?.toLocaleString('fa-IR') || '-'}</div><div className="mt-3 rounded-2xl bg-slate-950 p-3 text-sm text-slate-300">{task.address_text || 'آدرس ثبت نشده'}</div>{Array.isArray(task.service_items) && task.service_items.length > 0 && <div className="mt-2 flex flex-wrap gap-2">{task.service_items.map((service: { id?: string; title: string; labor_fee: number }) => <span key={service.id || service.title} className="rounded-full border border-amber-400/20 bg-amber-400/10 px-2 py-1 text-xs text-amber-100">{service.title} · {Number(service.labor_fee || 0).toLocaleString('fa-IR')} تومان</span>)}</div>}</div>
