      ...
      <section className="mt-5 grid gap-4 lg:grid-cols-[1fr_380px]">
        ...
        <aside className="space-y-4">
          ...
          <InfoPanel title="مسیر عملیات">
            ...
            <Step active={job.status === "on_way"} done={["on_way", "arrived", "working", "completed"].includes(String(job.status))} label="در مسیر مشتری" />
            <Step active={job.status === "arrived"} done={["arrived", "working", "completed"].includes(String(job.status))} label="رسیده به محل" />
            <Step active={job.status === "working"} done={["working", "completed"].includes(String(job.status))} label="در حال انجام" />
