...
export default function BookPage() {
  ...
  function next() {
    ...
    if (step === 2 && (!date || !selectedSlot)) return setError('روز و بازه زمانی دارای ظرفیت را انتخاب کنید.');
    if (step === 3 && !(selectedAddress || manualAddress.trim() || pickedLocation)) return setError('آدرس را وارد کنید یا موقعیت دقیق را روی نقشه انتخاب کنید.');
  ...
  async function submit() {
    ...
    try {
      ...
      const request = await createServiceRequest({
        customer_name: customerName.trim() || 'مشتری کارتل', customer_phone: customerPhone.trim(), customer_user_id: user?.id || null,
  ...
  return <main className="ct-book-page min-h-screen bg-slate-100 px-4 pb-28 pt-24 text-slate-900 sm:pt-28" dir="rtl"><div className="mx-auto max-w-6xl">
    <header className="ct-book-hero mb-4 rounded-[2rem] bg-gradient-to-l from-slate-900 via-slate-800 to-amber-700 p-5 text-white shadow-xl sm:mb-6 sm:p-7"><div className="flex items-center gap-3"><Wrench className="h-8 w-8 text-amber-300" /><div><h1 className="text-xl font-black sm:text-3xl">رزرو سرویس در محل</h1><p className="mt-2 text-sm text-white/65">خدمت، زمان و آدرس را انتخاب کن؛ هزینه قبل از ثبت شفاف محاسبه می‌شود.</p></div></div></header>
    <div className="ct-book-progress mb-4 grid grid-cols-4 gap-1.5 sm:mb-6 sm:gap-2">{['خدمت و خودرو','زمان','آدرس','تأیید نهایی'].map((label,index) => <div key={label} className={`rounded-xl px-1 py-2 text-center text-[10px] font-bold sm:rounded-2xl sm:px-3 sm:py-3 sm:text-sm ${step >= index+1 ? 'bg-amber-400 text-slate-950' : 'bg-white text-slate-400'}`}><span className="sm:hidden">{index+1}</span><span className="hidden sm:inline">{index+1}. {label}</span></div>)}</div>
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
      <section className="ct-book-form overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm sm:rounded-[2rem] sm:p-6">
        {step === 1 && <div className="space-y-6"><div><h2 className="mb-3 flex items-center gap-2 text-xl font-black"><Car className="h-5 w-5 text-amber-500" /> انتخاب خودرو</h2><button type="button" onClick={() => { setPendingCarId(selectedCarId); setCarSearch(''); setCarPickerOpen(true); }} className="ct-book-car-trigger flex w-full items-center justify-between rounded-2xl border border-slate-300 bg-slate-950 px-4 py-4 text-right text-white outline-none transition hover:border-amber-400"><span className="truncate font-bold">{selectedCar ? getCarTitle(selectedCar) : 'انتخاب خودرو'}</span><span className="text-xs text-amber-300">تغییر</span></button></div><div><h2 className="mb-3 text-xl font-black">خدمات موردنیاز</h2><div className="grid gap-3 md:grid-cols-2">{services.map((service) => { const active=selectedServiceIds.includes(service.id); return <button key={service.id} onClick={() => toggleService(service.id)} className={`rounded-2xl border p-4 text-right transition ${active ? 'border-amber-400 bg-amber-50 shadow-md' : 'border-slate-200 bg-slate-50 hover:border-slate-300'}`}><div className="flex items-start gap-3"><span className="text-2xl">{service.icon}</span><div className="flex-1"><div className="flex items-center justify-between"><b>{service.title}</b>{active && <Check className="h-5 w-5 text-emerald-500" />}</div><p className="mt-1 text-xs leading-6 text-slate-500">{service.description}</p><div className="mt-2 text-sm font-bold text-amber-700">اجرت {money(service.base_labor_fee)}</div></div></div></button>})}</div></div></div>}
        {step === 2 && <div className="space-y-6"><h2 className="flex items-center gap-2 text-xl font-black"><CalendarDays className="h-5 w-5 text-amber-500" /> انتخاب روز و ظرفیت</h2><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{dateOptions.map((item) => <button type="button" data-booking-slot aria-pressed={date===item.value} key={item.value} onClick={() => setDate(item.value)} className={`ct-booking-slot rounded-2xl border p-3 text-sm font-bold ${date===item.value?'is-selected border-amber-400':'border-slate-700'}`}>{item.label}</button>)}</div><div className="grid gap-3 md:grid-cols-3">{slots.map((slot) => <button type="button" data-booking-slot aria-pressed={slotId===slot.id} disabled={slot.remaining<=0} key={slot.id} onClick={() => setSlotId(slot.id)} className={`ct-booking-slot rounded-2xl border p-4 ${slotId===slot.id?'is-selected border-amber-400':'border-slate-700'} disabled:cursor-not-allowed disabled:opacity-45`}><Clock3 className="mx-auto mb-2 h-5 w-5"/><b>{slot.label}</b><p className={`mt-2 text-xs ${slot.remaining>1?'text-emerald-600':slot.remaining===1?'text-amber-600':'text-rose-600'}`}>{slot.remaining>0?`${slot.remaining.toLocaleString('fa-IR')} ظرفیت باقی‌مانده`:'تکمیل'}</p></button>)}</div></div>}
        {step === 3 && <div className="space-y-5"><h2 className="flex items-center gap-2 text-xl font-black"><MapPin className="h-5 w-5 text-amber-500" /> اطلاعات مشتری و آدرس</h2><div className="grid gap-4 md:grid-cols-2"><label className="space-y-2"><span className="text-sm font-bold">نام مشتری</span><div className="relative"><UserRound className="absolute right-4 top-4 h-4 w-4 text-slate-400"/><input value={customerName} onChange={(e)=>setCustomerName(e.target.value)} className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-3 pl-4 pr-11 outline-none focus:border-amber-400"/></div></label><label className="space-y-2"><span className="text-sm font-bold">شماره موبایل</span><div className="relative"><Phone className="absolute right-4 top-4 h-4 w-4 text-slate-400"/><input value={customerPhone} readOnly aria-readonly="true" className="w-full rounded-2xl border border-slate-200 bg-slate-100 py-3 pl-4 pr-11 font-bold text-slate-700 outline-none"/></div></label><label className="space-y-2"><span className="text-sm font-bold">کیلومتر فعلی</span><input type="number" value={currentKm} onChange={(e)=>setCurrentKm(e.target.value)} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-amber-400"/></label></div>{addresses.length>0 && <div className="grid gap-2">{addresses.map((address)=><button key={address.id} onClick={()=>{setAddressId(address.id||'');setCity(address.city)}} className={`rounded-2xl border p-4 text-right ${addressId===address.id?'border-amber-400 bg-amber-50':'border-slate-200 bg-slate-50'}`}><b>{address.title}</b><p className="mt-1 text-xs text-slate-500">{formatCustomerAddress(address)}</p></button>)}</div>}<textarea value={manualAddress} onChange={(e)=>setManualAddress(e.target.value)} rows={3} placeholder="پلاک، واحد و توضیحات تکمیلی آدرس" className="w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 outline-none focus:border-amber-400"/><MapLocationPicker initialLatitude={selectedAddress?.latitude || pickedLocation?.latitude} initialLongitude={selectedAddress?.longitude || pickedLocation?.longitude} onConfirm={(location)=>{setPickedLocation(location); if(location.address && !manualAddress.trim()) setManualAddress(location.address);}}/><textarea value={note} onChange={(e)=>setNote(e.target.value)} rows={2} placeholder="توضیحات تکمیلی" className="w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 outline-none focus:border-amber-400"/></div>}
      ...
      <div className="ct-book-car-modal-backdrop fixed inset-0 z-[100000] flex items-end justify-center bg-slate-950/80 p-3 backdrop-blur-sm sm:items-center" onMouseDown={(event) => { if (event.target === event.currentTarget) setCarPickerOpen(false); }}>
        <section className="ct-book-car-modal flex max-h-[78dvh] w-full max-w-lg flex-col overflow-hidden rounded-[1.75rem] border-2 border-amber-400 bg-slate-950 text-white shadow-2xl" dir="rtl">
          <header className="flex items-center justify-between border-b border-white/10 p-4">
            <div><h2 className="font-black">انتخاب خودرو</h2><p className="mt-1 text-xs text-white/55">نام برند یا مدل را جستجو کن.</p></div>
            <button type="button" onClick={() => setCarPickerOpen(false)} className="grid h-10 w-10 place-items-center rounded-xl bg-white/10" aria-label="بستن"><X className="h-5 w-5" /></button>
          </header>
          <div className="p-3">
